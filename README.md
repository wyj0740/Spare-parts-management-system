...
### 📄 Windows文件夹管理
- **资源管理器集成**：每个备件自动创建独立文件夹
- **直接操作**：点击按钮在Windows资源管理器中打开文件夹
- **批量初始化**：一键为所有备件创建文件夹
- **历史文档库**：独立文件夹存放历史文档
- **状态显示**：主页显示文件夹状态和文件数量
...
│
├── files/                          # 文件管理目录（自动创建）
│   ├── spare_parts/               # 备件文件夹（按资产编号命名）
│   └── historical_documents/      # 历史文档文件夹
│
├── backups/                        # 备份文件目录（自动创建）
├── logs/                           # 运行日志目录（自动创建）
...
max_upload_size_mb = 100  # 备份文件最大100MB
```
...
2. **端口占用**：默认使用5000端口，如被占用请修改config.ini
3. **单实例运行**：同一时间只能运行一个程序实例
...
### v2.1 (2026-02-06) - 文件管理重构版

**🔄 架构重构**
- ✅ 文件管理改用Windows文件夹：每个备件自动创建独立文件夹
- ✅ 历史文档改用独立文件夹管理
- ✅ 删除废弃的attachments和historical_documents数据库表
- ✅ 单实例运行：防止重复打开程序
